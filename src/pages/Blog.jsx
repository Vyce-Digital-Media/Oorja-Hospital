...
export default function Blog() {
  ...
  // Separate featured article if we are in "All" view with no search query
  const showFeatured = activeCategory === "All" && searchQuery.trim() === "";
  const featuredArticle = showFeatured ? blogArticles.find(art => art.featured) || blogArticles[0] : null;
  const gridArticles = showFeatured ? filteredArticles.filter(art => art.id !== featuredArticle?.id) : filteredArticles;
  ...
  return (
    <div className="pt-20 sm:pt-24 overflow-hidden bg-slate-50 text-slate-900 min-h-screen">
      ...
      <section className="py-12 sm:py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Featured Hero Article Card (Only when All category & no search) */}
        {showFeatured && featuredArticle && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
            className="mb-14 bg-white rounded-3xl border border-slate-200/80 shadow-xl overflow-hidden group hover:shadow-2xl transition-all duration-300"
          >
            <div className="grid grid-cols-1 lg:grid-cols-12">
              {/* Left Image */}
              <div className="lg:col-span-7 relative overflow-hidden min-h-[300px] sm:min-h-[400px]">
                <img
                  src={featuredArticle.image}
                  alt={featuredArticle.title}
                  className="w-full h-full object-cover object-center group-hover:scale-105 transition-all duration-700"
                />
                <div className="absolute top-4 left-4 bg-teal-500 text-white text-xs font-extrabold uppercase px-3.5 py-1.5 rounded-full shadow-md flex items-center gap-1.5">
              ...
              <div className="lg:col-span-5 p-8 sm:p-10 lg:p-12 flex flex-col justify-between bg-gradient-to-b from-white to-slate-50/50">
                <div>
                  <div className="flex items-center gap-4 text-xs font-semibold text-slate-500 mb-4">
                    <span className="bg-teal-50 text-teal-700 px-3 py-1 rounded-lg font-bold">
                  ...
                  <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 group-hover:text-teal-600 transition-colors leading-tight mb-4">
                ...
                <div className="pt-6 border-t border-slate-200/60 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-500 line-clamp-1">{featuredArticle.author.role}</p>
                    </div>
                  </div>

                  <button
                    onClick={() => setSelectedArticle(featuredArticle)}
                    className="px-5 py-2.5 bg-teal-600 hover:bg-teal-700 text-white rounded-xl text-xs font-bold flex items-center gap-2 transition-all duration-200 group-hover:translate-x-1 shadow-sm"
                  >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {gridArticles.map((art, idx) => (
            <motion.div
              ...
              className="bg-white rounded-3xl border border-slate-200/80 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col group cursor-pointer hover:-translate-y-1.5"
            >
              {/* Card Thumbnail */}
              <div className="relative aspect-[16/10] overflow-hidden bg-slate-100">
                ...
                />
                <div className="absolute top-3 left-3 bg-white/90 backdrop-blur-md text-slate-900 text-xs font-bold px-3 py-1 rounded-lg shadow-sm">
                  {art.category}
                </div>
              </div>
